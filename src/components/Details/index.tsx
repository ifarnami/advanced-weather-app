...
const Details: React.FC<IDetailsProps> = (): JSX.Element => {
  ...
  return (
    <div className="pt-4 mt-20 lg:mt-0 flex flex-col h-screen items-center backdrop-blur-lg bg-white bg-opacity-[0.04] border-t-[5px] border-t-white border-opacity-[0.14]">
      ...
      <p className="font-medium uppercase mt-5 text-sm">
      ...
      <div className="flex w-full justify-around items-center mt-4">
        ...
        <ul className="flex flex-col gap-6">
          <li className="flex items-center justify-between gap-2">
            <p>{weatherContext?.weatherData.temp_max} &deg;</p>
            {Icons.maxTemp()}
          </li>
          <li className="flex items-center justify-between gap-2">
            <p>{weatherContext?.weatherData.temp_min} &deg;</p>
            {Icons.minTemp()}
          </li>
          <li className="flex items-center justify-between gap-2">
            <p>{weatherContext?.weatherData.humidity}%</p>
            {Icons.humidity()}
          </li>
          <li className="flex items-center justify-between gap-2">
            <p>{weatherContext?.weatherData.clouds}%</p>
            {Icons.cloud()}
          </li>
          <li className="flex items-center justify-between gap-2">
            <p>
              {weatherContext?.weatherData.speed}
